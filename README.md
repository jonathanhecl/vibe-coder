...
1. **Assisted Command Execution Mode (`--assisted-yes`, `/yes assisted`, `/jevstyle assisted on`)**:
   ...
   - **Auto-approval notice**: Whenever assisted mode approves an action without prompting, the terminal prints a `✓ <tool> approved by JEV Style assisted mode (<latency>)` line — for example `(70ms)` — so it is clear why it ran and how long the decision took.
