# Mission guide: batch processing through a local job API

You are running a long, unattended batch mission. Follow this guide exactly.
...
- On start, if `results.jsonl` exists, read it and SKIP every item whose `id`
  already has a `verified` line. This makes the mission resumable after a crash.
